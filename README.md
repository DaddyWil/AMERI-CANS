...
Fifty flat files. No build step, no folders. Keep them side by side at the repository root.

- index.html: landing page (https://williamhans.app/), a minimal index of three rows: Products (#products; Powered equipment and Water systems tabs, MARK IV expands to its product page, Site Guide, and rough 3D model), Cut sheets (#cut-sheets; the combined PDF, the MARK IV sheet, and one per product), and Internal tools (#internal; #calculators, #site-survey, and #branding land there too). Internal tools asks for the team password; Branding lives inside it.
...
- 8-stall.html, 10-station.html, 4-stall.html, ada-2.html, 8-stall-shower.html, water-station.html, bootwash.html: product pages for the powered equipment line (from the December 2025 cut sheets); pump-house.html: 500 Gal Pump House (from the Texas Operations water system diagram, September 2026, not in the cut sheet PDF; its hero is the 3D viewer); pump-house-viewer.html and pump-house-model.html: the pump house rough 3D model (embedded viewer, and the standalone page with door, night, trailer scene, and OBJ/GLB download); og-<name>.jpg is each page's link preview image
...
- work-order-intake.html, water-waste.html, install-estimator.html, materials-pricing.html, site-survey-universal.html, site-survey-multiplex.html, maps.html, interview-form.html, fsl-interview.html: internal staff tools, stored encrypted; each shows only a password screen until unlocked
- wash.js: shared background wash (faded service flags and brand marks) and the small animation set, loaded by every page except the 3D viewers and the game
